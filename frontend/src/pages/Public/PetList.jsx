...
const PetList = () => {
  const [allPets, setAllPets] = useState([]); // full dataset
  const [pets, setPets] = useState([]); // filtered list
  const [page, setPage] = useState(1);
  const [search, setSearch] = useState("");
  const [filters, setFilters] = useState({ species: "", breed: "", age: "" });
  ...
  const fetchPets = async () => {
    try {
      ...
      // Generate options from full dataset
  ...
  useEffect(() => {
    ...
    if (filters.breed) filtered = filtered.filter(p => p.breed === filters.breed);
    if (filters.age) filtered = filtered.filter(p => Number(p.age) === Number(filters.age));
  ...
  const clearFilters = () => setFilters({ species: "", breed: "", age: "" });
      ...
      <div className="row mb-3 g-2">
        ...
        <div className="col-md-2 d-flex gap-2">
          <button className="btn btn-outline-primary w-100"
              disabled={!filters.species && !filters.breed && !filters.age} // disabled if no filters applied
        ...
        {paginatedPets.length === 0 ? (
          ...
          paginatedPets.map((pet) => (
            ...
            >
              ...
              <Link to={`/pets/${pet.id}`} className="btn btn-primary btn-sm">
                View Details
              </Link>
            </div>
          ))
